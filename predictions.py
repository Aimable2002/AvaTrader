...
import pandas as pd
# from trackLearn import monitor
...
def prepare_forex_data(df, symbol, sequence_length=10):
    ...
    data['MA_15'] = data['close'].rolling(window=5).mean()  # 5-day Moving Average
    ...
    for symbol in symbols:
        try:
            ...
            X_train, X_val, y_train, y_val = train_test_split(
                X, y, test_size=0.2, shuffle=True  # No shuffle for time series  False
            )
            ...
            with torch.no_grad():
                ...
                confidence_score = calculate_confidence(predicted_scaled)  
                
                # Inverse transform
                dummy = np.zeros((1, input_size))
                dummy[:, 4] = predicted_scaled.cpu().numpy().flatten()
                predicted_price = scaler.inverse_transform(dummy)[0, 4]
                
                current_price = df[symbol]['close'].iloc[-1]
                # print(f"\n Print the last df row: {df[symbol].iloc[-1]}")

                moving_average = df[symbol]['close'].rolling(window=5).mean().iloc[-1]
...
def calculate_confidence(predicted_scaled):
    """
    Calculate a confidence score based on the model's output.
    This is a placeholder function; implement your logic here.
    """
    # Example: return a random confidence score for demonstration
...
def train_model(model, train_loader, val_loader, symbol, epochs=100, learning_rate=0.0001):
    """
    ...
    #optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=0.005)  # L2 Regularization
    ...
    patience = 10  # Early stopping patience 20
    patience_counter = 0
    min_delta = 0.0001 
    ...
    learning_rates = []
    val_accuracies = []
    sharpe_ratios = []   # Store Sharpe Ratios for plotting
    ...
    for epoch in range(epochs):
        ...
        # monitor.plot_learning_progress(avg_train_loss, avg_val_loss)
    ...
    plt.subplot(4, 1, 4)
    plt.plot(directional_accuracies, label='Directional Accuracy', color='purple')
...
# #                 input_size = X.shape[2]
# #                 print(f"Symbol: {symbol}, X Shape: {X.shape}, Input Size: {input_size}")
# #             else:
# #                 print(f"Symbol: {symbol}, X is empty, skipping...")
# #                 continue
...
# #                 # Inverse transform
# #                 dummy = np.zeros((1, input_size))
# #                 dummy[:, 4] = predicted_scaled.cpu().numpy().flatten() # 3
# #                 predicted_price = scaler.inverse_transform(dummy)[0, 4]

# #                 #predicted_class = (predicted_scaled.cpu().numpy() > 0.5).astype(int) # fro classification convert to binary
...
# #                 #     'current_price': current_price,
# #                 #     'predicted_price': predicted_price,
# #                 #     'predicted_class': predicted_class[0][0],  # 1 for up, 0 for down
# #                 #     'movement': 'UP' if predicted_class[0][0] == 1 else 'DOWN',
# #                 #     'change_percent': ((predicted_price - current_price) / current_price) * 100
# #                 # }
